...
app.on("before-quit", (event) => {
  /*
   * Гость не должен закрыть блокировку сам, поэтому обычный выход отклоняется.
   * Но обновление ставится поверх работающего агента, и установщик просит его
   * закрыться: раньше он получал отказ и вставал с «Не удалось закрыть». Теперь
   * закрывать агента при обновлении умеет сам установщик — он снимает процесс и
   * придерживает сторожа, — а эта защита осталась ровно для того, для чего
   * писалась: для гостя за клавиатурой.
   */
